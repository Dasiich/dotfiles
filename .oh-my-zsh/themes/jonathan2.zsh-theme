...
${PR_YELLOW}%${PR_PWDLEN}<...<%~%<<\
${PR_GREY})$(virtualenv_prompt_info)$(ruby_prompt_info)$(conda_prompt_info)${PR_GREY}${PR_HBAR}${PR_HBAR}${(e)PR_FILLBAR}${PR_HBAR}${PR_GREY}(\
${PR_CYAN}%(!.%SROOT%s.%n)${PR_GREY}@${PR_GREEN}%m\
${PR_GREY})${PR_GREY}${PR_HBAR}${PR_URCORNER}\

${PR_GREY}${PR_LLCORNER}${PR_GREY}${PR_HBAR}(\
${PR_MAGENTA}%D{%H:%M:%S}\
${PR_NO_COLOUR}$(git_prompt_info)$(git_prompt_status)${PR_BLUE})${PR_CYAN}${PR_HBAR}\
${PR_HBAR}\
>${PR_NO_COLOUR} '
